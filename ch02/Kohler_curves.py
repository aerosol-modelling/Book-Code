...
if option == 1:
    ...
    for size in dry_size_list:
        ...
        mass_particle = (4.0/3.0)*np.pi*np.power((0.5*size*1.0e-9),3.0)*density_solute #kg
        ...
        Sw_list = []
        droplet_size_list =[]
        # Now iterative through droplet sizes. To do this we add 1nm to the initial
        # dry particle size until we reach 10'000nm.
        ...
        while droplet_size <= 10000.0:
            ...
            Sw=Kohler_theory_modules.Kohler_curve(Dp,Temp_K,moles_solute,diss_num,molar_vol_solute,molar_vol_water,density_solute,density_water,molar_weight_water,surf_tens,R_gas)
            Sw_list.append(Sw)
        ...
        plt.plot(np.array(droplet_size_list),(np.array(Sw_list)-1.0)*100.0,label='%s nm' % size)
    ...
    for size in dry_size_list:
        ...
        mass_particle = (4.0/3.0)*np.pi*np.power((0.5*Dp),3.0)*density_solute #kg
        moles_solute = (mass_particle*1.0e3)/molar_weight_solute #moles

        # Calculate the critical saturation ratio by Brents method in minimize_scalar
        result = minimize_scalar(Kohler_theory_modules.Kohler_curve_min, bounds=(size, upper_limit), args=(Temp_K,moles_solute,stoich_coeff,molar_vol_solute,molar_vol_water,density_solute,density_water,molar_weight_water,surf_tens,R_gas),method='bounded')
        ...
        Sc_it, Dpc_it = Kohler_theory_modules.Crit_2param(Dp,Temp_K,moles_solute,surf_tens,diss_num,molar_vol_water,density_water,molar_weight_water,R_gas)
    ...
    for vol_frac in volume_frac_list:
        ...
        while droplet_size <= 10000.0:
            ...
            # Call the Kohler_curve function
            Sw=Kohler_theory_modules.Kappa_kohler(Dp,dry_diameter*1.0e-9,Temp_K,solubility_C,Kappa_array,surf_tens,vol_frac_array,density_water,molar_weight_water,R_gas)
            Sw_list.append(Sw)
        ...
        # Plot the results on the figure
        # Note we are plotting supersaturation levels
        plt.plot(np.array(droplet_size_list),(np.array(Sw_list)-1.0)*100.0,label=r'%s' % vol_frac)
    # Constrain the y axis to show supersaturation
    plt.xscale('log')
    plt.ylim(-1.0, 0.7)
    ax = plt.gca()
