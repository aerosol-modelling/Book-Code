...
def main():
    ...
    index = 1 
    t1 = time.time()
    M =  np.zeros((N_part,N_part),dtype=int) #### Added
    ...
    return(times, hist[0:len(times),:],M)

fig = plt.figure(figsize=(4.5,3.5))
axes = fig.add_subplot(1,1,1)
max_size_plot = 8 
times, histogram, M = main()
...
fig = plt.figure()
axes = fig.add_subplot(1,1,1)
NC = np.mean(num_coags_total,axis=0)
norm = matplotlib.colors.LogNorm(vmin=1, vmax=10**np.ceil(np.log10(np.max(NC))))
cbar = axes.pcolormesh(NC,cmap='cool',norm=norm)
axes.set_xlabel('bin $i$')
axes.set_ylabel('bin $j$')
lims = [0,np.ceil(np.max(np.nonzero(NC))/10)*10]
axes.set_xlim(lims)
axes.set_ylim(lims)
c = fig.colorbar(cbar,label='Number of coagulation events')
fig.savefig('ch05_q08.pdf')
