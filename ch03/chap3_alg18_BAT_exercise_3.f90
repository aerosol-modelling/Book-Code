!****************************************************************************************
!*   :: Purpose ::                                                                      *
!*   Chapter 3, exercise 3 solution for determining the activity coefficients and       *
!*   equilibrium saturation vapor pressures.                                            *
!*                                                                                      *
!*   :: Authors ::                                                                      *
...
!for exercise 3: (Raoult's law equilibrium gas phase mass conc.):
!(2.1): set input parameters for the organic component, 2-methyltetrol-dimer, C10H22O7, SMILES: CC(O)C(C)(O)C(O)OCC(O)C(C)(O)CO
...
M_org       = 254.28_wp                         ![g/mol], molar mass
OtoC        = 7.0_wp/10.0_wp                    ![-], O:C ratio
HtoC        = 22.0_wp/10.0_wp
...
!       with a few points:
np = 101 !15                                    !set the number of BAT evaluation points (> 5) for x_org within [0.0, 1.0]
...
write(*,'(A,I0,/)') '## Exercise 3 solution -- consideration of LLPS ##'
...
block
    ...
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.75_wp, legend_position=3, &
            & metafile='pdf', out_file_name='exerc3_activity_curves')
end block
...
block
    ...
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.5_wp, legend_position=3, &
            & metafile='pdf', out_file_name='exerc3_Delta_Gmix')
end block
...
block
    ...
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.3_wp, legend_position=3, &
        & metafile='pdf', out_file_name='exerc3_equil_Delta_Gmix')
    !------------------------------------------------------------
    ...
    if (phase_sep_detected) then
        ...
        call dislin_plot(xlabel, ylabel, yaxis_mod=0.25_wp, legend_position=3, &
            & metafile='pdf', out_file_name='exerc3_equil_fracA')
    endif
    !------------------------------------------------------------
    ...
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.3_wp, legend_position=3, &
        & metafile='pdf', out_file_name='exerc3_equil_activity_curves')
    !------------------------------------------------------------
    ...
    if (phase_sep_detected) then
        ...
        call dislin_plot(xlabel, ylabel, yaxis_mod=0.25_wp, legend_position=3, &
            & metafile='pdf', out_file_name='exerc3_equil_qA')
    endif
    !------------------------------------------------------------ 
    ...
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.6_wp, legend_position=3, &
        & metafile='pdf', out_file_name='exerc3_equil_xwtot')
    !------------------------------------------------------------ 
    ...
    if (phase_sep_detected) then
        ...
        xlabel = 'mole fraction of organic, $x_{\rm org}$'
        ylabel = 'ln(activity coefficient)'
    endif
    ...
    ylabel = 'equil. ln(activity coefficient)'
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.5_wp, legend_position=3, &
        & metafile='pdf', out_file_name='exerc3_equil_lnActCoeff')
    !------------------------------------------------------------ 
    
    !(g) for exercise 3a; plot equilibrium state activity coefficients
    if (phase_sep_detected) then
        !water:
        call add_plot_xydata(xv=x_org_vec(1:iloc2), yv=exp(ln_actcoeff(1:iloc2,1)), & 
            & ltext='water act. coeff., $\gamma_w^\alpha$', &
            & pen_wid=6.0_wp, rgb_col=rgb_blue, lstyle='solid', plot_symb='curve')
        call add_plot_xydata(xv=x_org_vec(iloc1:), yv=exp(ln_actcoeff_B(iloc1:,1)), &
            & ltext='water act. coeff., $\gamma_w^\beta$', &
            & pen_wid=4.0_wp, rgb_col=rgb_blue, lstyle='dashed_medium', plot_symb='curve')
        !organic:
        call add_plot_xydata(xv=x_org_vec(1:iloc2), yv=exp(ln_actcoeff(1:iloc2,2)), &
            & ltext='org. act. coeff., $\gamma_{\rm org}^\alpha$', &
            & pen_wid=6.0_wp, rgb_col=rgb_green, lstyle='solid', plot_symb='curve')
        call add_plot_xydata(xv=x_org_vec(iloc1:), yv=exp(ln_actcoeff_B(iloc1:,2)), &
            & ltext='org. act. coeff., $\gamma_{\rm org}^\beta$', &
            & pen_wid=4.0_wp, rgb_col=rgb_green, lstyle='dashed_medium', plot_symb='curve')
        ...
        !water:
        call add_plot_xydata(xv=x_org_vec, yv=exp(ln_actcoeff(:,1)), & 
            & ltext='water act. coeff., $\gamma_w$', &
            & pen_wid=6.0_wp, rgb_col=rgb_blue, lstyle='solid', plot_symb='curve')
        !organic:
        call add_plot_xydata(xv=x_org_vec, yv=exp(ln_actcoeff(:,2)), &
            & ltext='org. act. coeff., $\gamma_{\rm org}$', &
            & pen_wid=6.0_wp, rgb_col=rgb_green, lstyle='solid', plot_symb='curve')        
    ...
    !set overall plot properties and generate plot:
    call dislin_plot(xlabel, ylabel, yaxis_mod=0.5_wp, legend_position=3, &
        & metafile='pdf', out_file_name='exerc3_equil_ActCoeff')
    !------------------------------------------------------------
    
end block  


!for exercise 3a; output value ranges for the activity coefficients (for exercise 3)
write(*,*) ''
write(*,'(A)') '3a)'
!water:
i = minloc(ln_actcoeff(:,1), dim=1)
k = maxloc(ln_actcoeff(:,1), dim=1)
write(*,'(4(A,ES12.5,1X))') 'water activity coeff. varies from ', exp(ln_actcoeff(i,1)), 'at x_org = ', x_org_vec(i), &
    & 'to ', exp(ln_actcoeff(k,1)), 'at x_org = ', x_org_vec(k)
!organic:
i = minloc(ln_actcoeff(:,2), dim=1)
k = maxloc(ln_actcoeff(:,2), dim=1)
write(*,'(4(A,ES12.5,1X))') 'organic activity coeff. varies from ', exp(ln_actcoeff(i,2)), 'at x_org = ', x_org_vec(i), &
    & 'to ', exp(ln_actcoeff(k,2)), 'at x_org = ', x_org_vec(k)
!------------------------------------------------------------
    
!for exercise 3b; compute gas phase saturation mass concentrations
block
    real(wp),parameter :: Rgas = 8.314462_wp    ![J /(K mol)]
    real(wp) :: x_org
    real(wp),dimension(2) :: C0sat, Cg, Mmass, psat
    !....................................
    
    !set pure-component saturation vapor pressures for T = 293.15 K:
    psat(1) = 2336.7_wp                     ![Pa] water
    psat(2) = 1.034E-9_wp                   ![Pa] 2-methyltetrol dimer
    Mmass = [18.015_wp, M_org]              ![g/mol]
    C0sat = psat*Mmass*1.0E6_wp / (Rgas * 293.15_wp)    ![ug/m^3]  
    
    !compute activity coefficients for x_org = 0.3
    x_org = 0.3_wp
    call BAT_light(x_org, M_org, OtoC, density_org, ln_actcoeff(1,:), activities(1,:) )
    !apply mod. Raoult's law:
    Cg = C0sat*activities(1,:)            ![ug/m^3] for water (1) and organic (2)
    write(*,*) ''
    write(*,'(A,F5.2,A)') '3b)  At 20 C and for x_org = ', x_org,' :'
    write(*,'(A,2(ES12.5,",",1X))') 'C0sat(water), C0sat(org) [ug/m^3] = ', C0sat(:)
    write(*,'(A,2(ES12.5,",",1X))') 'Cg(water), Cg(org)       [ug/m^3] = ', Cg(:)
    write(*,*) ''
    write(*,'(A)') 'for comparison, assuming ideal mixing:'
    write(*,'(A,2(ES12.5,",",1X))') 'Cg_id(water), Cg_id(org) [ug/m^3] = ', C0sat(1)*(1.0_wp - x_org), C0sat(2)*x_org
    
end block

end subroutine BAT_exercise_3