...
# Version of the RHS function with no ozone effects
def dy_dt(array,t):
    ...
    for size_step in range(num_bins):
        ...
        (N_per_bin[size_step]*1.0e6)))/(4.0*np.pi*density))**(1.0/3.0)
...
# Version of the RHS function with ozone effects
def dy_dt_ozone(array,t):
    ...
    # Now write out the RHS for each volatility bin according to our simple
    # mechanism
...
solution_no_ozone = odeint(dy_dt_numba, array, t, rtol=1.0e-6, atol=1.0e-4, tcrit=None)
solution_ozone = odeint(dy_dt_numba_ozone, array, t, rtol=1.0e-6, atol=1.0e-4, tcrit=None)

# Now extract a size distribution for every point in the simulation
size_array_matrix_no_ozone = np.zeros((1000,num_bins), dtype=float)
...
for step in range(solution_no_ozone.shape[0]):
    ...
    condensable_mass = 0
    for size_step in range(num_bins):
        temp_array=solution_no_ozone[step,num_species+size_step*num_species:num_species+num_species*(size_step+1)]
        total_moles=np.sum(temp_array)+core_concentration[size_step]
        mole_fractions=temp_array/total_moles
        density_array = np.zeros((num_species+1), dtype=float)
        density_array[0:num_species]=density_org[0:num_species]
        ...
        mass_fractions_array=mass_array/total_mass
        density=1.0/(np.sum(mass_fractions_array/density_array))
        size_array_matrix_no_ozone[step,size_step]=((3.0*((total_mass*1.0e3)/(N_per_bin[size_step]*1.0e6)))/(4.0*np.pi*density))**(1.0/3.0)
...
for step in range(solution_ozone.shape[0]):
    ...
    for size_step in range(num_bins):
        temp_array=solution_ozone[step,num_species+size_step*num_species:num_species+num_species*(size_step+1)]
        total_moles=np.sum(temp_array)+core_concentration[size_step]
        mole_fractions=temp_array/total_moles
        density_array = np.zeros((num_species+1), dtype=float)
        density_array[0:num_species]=density_org[0:num_species]
...
plt.plot(t, size_array_matrix_no_ozone*2.0e6, 'r-')
