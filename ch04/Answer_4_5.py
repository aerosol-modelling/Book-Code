...
#---------------------------
# 
# The following functions were created for UManSysProp and are given for the purpose of the exercise
# 
...
SIMPOL_SMARTS = _read_smarts('4_5_SIMPOL.smarts')

#------------------------------------------------------ 
# define function to search for groups

def simpol_groups (compound):
    m = matches(SIMPOL_SMARTS, compound)
    result = {}
    # Zeroth group (constant used in all calculations)
    result['0'] = 1
    # total number of carbons present
    result['1'] = m[1]
    # Hydroxyl group bonded to NON-aromatic carbon
    result['2'] = m[2]
    # carboxylic acid
    result['3'] = m[3]
...
SIMPOL_1 = _read_data('4_5_SIMPOL.data', value_col=1)
SIMPOL_2 = _read_data('4_5_SIMPOL.data', value_col=2)
SIMPOL_3 = _read_data('4_5_SIMPOL.data', value_col=3)
SIMPOL_4 = _read_data('4_5_SIMPOL.data', value_col=4)
...
def simpol_vapour_pressure(compound, temperature):
    m = simpol_groups(compound)
    
    b1 = aggregate_matches(m, SIMPOL_1)
    b2 = aggregate_matches(m, SIMPOL_2)	
    b3 = aggregate_matches(m, SIMPOL_3)
    b4 = aggregate_matches(m, SIMPOL_4)
...
# The following code block is taken from the example folder of UManSysProp and can be used 
# to read the SMILES of multiple compounds using a single input.
...
#---------------------------------------
#
# calculate vapour pressure log10(atm)
#
#---------------------------------------
